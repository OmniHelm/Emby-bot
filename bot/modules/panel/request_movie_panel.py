...
@bot.on_callback_query(filters.regex('request_by_link') & user_in_group_on_filter)
async def request_by_link(_, call):
    """通过 TMDB 链接求片"""
    if not moviepilot.status:
        return await callAnswer(call, '❌ 管理员未开启点播功能', True)
    ...
    await callAnswer(call, '📎 链接求片')
    await editMessage(call,
                      f"📎 通过 TMDB 链接求片\n\n"
                      f"请在120s内发送 TMDB 链接（电影或剧集）\n"
                      f"例如: https://www.themoviedb.org/movie/12345\n\n"
                      f"退出点 /cancel",
                      buttons=re_download_center_ikb)
    ...
    # 解析 TMDB 链接
    link = txt.text.strip()
    if not is_valid_tmdb_link(link):
        await txt.delete()
        await editMessage(call,
                         '❌ 无效的 TMDB 链接\n\n'
                         '请确保链接格式正确，例如:\n'
    ...
    parsed = parse_tmdb_link(link)
    if not parsed:
        await txt.delete()
        await editMessage(call, '❌ 解析链接失败，请重试', buttons=re_download_center_ikb)
        return
    ...
    # 格式化标题
    title = format_title_slug(title_slug) if title_slug else f"TMDB_{tmdb_id}"
    media_type_cn = get_media_type_cn(media_type)

    await txt.delete()
    await editMessage(call, f'🔍 正在查询 Emby 库中是否已有《{title}》...')
    ...
    try:
        ...
        if emby_results:
            # Emby 库中已存在
            text = f"✅ Emby 库中已存在相关资源:\n\n"
            for item in emby_results[:5]:  # 最多显示5个结果
                text += f"• {item['title']} ({item['year']})\n"
            text += f"\n您求的《{title}》可能已经在库中了！"
            await editMessage(call, text, buttons=re_download_center_ikb)
            return

        # Emby 库中不存在，记录求片请求
        download_id = f"tmdb_{media_type}_{tmdb_id}"
        detail = f"链接: {link}\n类型: {media_type_cn}"

        # 添加到数据库
        success = sql_add_request_record(
        ...
        if not success:
            await editMessage(call, '❌ 记录求片失败，请联系管理员', buttons=re_download_center_ikb)
            return

        # 通知管理员
        admin_text = (
            f"📢 新求片请求\n\n"
            f"用户: {call.from_user.first_name} ({call.from_user.id})\n"
            f"片名: {title}\n"
            f"类型: {media_type_cn}\n"
            f"TMDB ID: {tmdb_id}\n"
            f"链接: {link}\n\n"
        ...
        try:
            # 优先使用 movie_request_chatid，否则使用第一个授权群组
            from bot import group
            chat_id = moviepilot.movie_request_chatid if moviepilot.movie_request_chatid else (group[0] if group and len(group) > 0 else None)

            if chat_id:
                await sendMessage(call, admin_text, send=True, chat_id=chat_id)
                LOGGER.info(f"求片通知已发送到群组 {chat_id}")
            else:
                LOGGER.warning("未配置求片通知群组，无法发送通知")
        except Exception as e:
            LOGGER.error(f"发送求片通知失败: {str(e)}")
        ...
        user_text = (
            ...
            f"类型: {media_type_cn}\n"
            f"TMDB ID: {tmdb_id}\n\n"
            f"管理员收到通知后会尽快处理，请耐心等待！"
        )
        await editMessage(call, user_text, buttons=re_download_center_ikb)

    except Exception as e:
        LOGGER.error(f"链接求片处理失败: {str(e)}")
        await editMessage(call, f'❌ 处理失败: {str(e)}', buttons=re_download_center_ikb)