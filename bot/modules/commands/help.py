"""
帮助命令模块
提供 /help 和 /faq 命令
"""

from pyrogram import filters
from bot import bot, prefixes, credits
from bot.func_helper.msg_utils import sendMessage, deleteMessage
from bot.func_helper.filters import user_in_group_on_filter
from bot.func_helper.fix_bottons import ikb
from bot.constants.messages import Messages


# /help 命令 - 显示帮助信息
@bot.on_message(filters.command('help', prefixes) & user_in_group_on_filter)
async def help_command(_, msg):
    """显示帮助信息和命令列表"""
    await deleteMessage(msg)

    help_text = f"""
📚 **EmbyBot 帮助中心**

╭─────────────────╮
│  💡 **快速帮助**
╰─────────────────╯

**常用命令：**
• `/start` - 打开主面板
• `/help` - 查看本帮助
• `/faq` - 常见问题解答
• `/exchange` - 兑换注册码/续期码
• `/myinfo` - 查看我的信息（群组）
• `/count` - 查看媒体统计

---

**快速开始：**

**1️⃣ 创建账户**
   主面板 → 🎨 创建账户
   输入：`用户名 安全码`
   示例：`张三 1234`

**2️⃣ 登录 Emby**
   下载 Emby 客户端
   使用账户信息登录
   开始观看影片

**3️⃣ 续期账户**
   使用续期码：`/exchange 续期码`
   或在兑换商店兑换

---

**用户功能：**
• 💖 我的收藏 - 查看收藏的影片
• 💠 我的设备 - 管理登录设备
• 🔑 重置密码 - 忘记密码时使用
• 🎬 显示/隐藏 - 自定义媒体库
• 🏪 兑换商店 - 使用{credits}兑换

---

**账户规则：**
• 用户名：支持中英文，2-20字符
• 安全码：4-6位数字
• 同时播放：最多2台设备
• 密码：仅显示一次，请妥善保管

---

**获取{credits}：**
• 🎯 每日签到
• ✅ 完成任务
• 👥 邀请好友
• 🎊 参与活动

---

**需要更多帮助？**

点击下方按钮查看详细文档
"""

    # 创建帮助按钮
    help_buttons = ikb([
        [('📖 用户指南', 'help_guide', 'url' if False else None),
         ('❓ 常见问题', 'help_faq')],
        [('🎯 快速开始', 'help_quickstart'),
         ('💬 联系管理', 'help_contact')],
        [('❌ 关闭', 'closeit')]
    ])

    await sendMessage(msg, help_text, buttons=help_buttons, timer=180)


# /faq 命令 - 常见问题
@bot.on_message(filters.command('faq', prefixes) & user_in_group_on_filter)
async def faq_command(_, msg):
    """显示常见问题解答"""
    await deleteMessage(msg)

    faq_text = f"""
❓ **常见问题解答 (FAQ)**

╭─────────────────╮
│  🔍 **热门问题**
╰─────────────────╯

**Q1: 如何创建 Emby 账户？**
A: `/start` → 🎨 创建账户 → 输入 `用户名 安全码`

**Q2: 忘记密码怎么办？**
A: 主面板 → 👤 用户功能 → 🔑 重置密码

**Q3: 如何续期账户？**
A: 使用命令 `/exchange 续期码`
   或在 🏪 兑换商店 使用{credits}兑换

**Q4: 账户到期后会怎样？**
A: • 无法登录 Emby 服务器
   • 数据保留 7 天
   • 7 天内续期可恢复

**Q5: 可以在多个设备上使用吗？**
A: • 可以在多个设备登录
   • 同时播放限制 2 台设备
   • 查看设备：💠 我的设备

**Q6: 视频卡顿怎么办？**
A: 1️⃣ 降低画质到 720P/480P
   2️⃣ 开启转码播放
   3️⃣ 更换服务器线路
   4️⃣ 检查网络速度

**Q7: 如何获取{credits}？**
A: • 🎯 每日签到
   • ✅ 完成任务
   • 👥 邀请好友注册
   • 🎊 参与群组活动

**Q8: 忘记安全码怎么办？**
A: 只能联系管理员协助重置
   提供 Telegram ID 验证身份

**Q9: 可以修改用户名吗？**
A: 不可以，用户名创建后无法修改
   只能删除旧账户重新创建

**Q10: 如何删除账户？**
A: 主面板 → 👤 用户功能 → 🗑️ 删除账号
   ⚠️ 此操作无法撤销

---

**更多问题？**

点击下方按钮查看完整 FAQ
"""

    faq_buttons = ikb([
        [('📚 完整 FAQ', 'faq_full'),
         ('📖 用户指南', 'help_guide')],
        [('💬 联系管理员', 'help_contact'),
         ('❌ 关闭', 'closeit')]
    ])

    await sendMessage(msg, faq_text, buttons=faq_buttons, timer=180)


# /guide 命令 - 用户指南
@bot.on_message(filters.command('guide', prefixes) & user_in_group_on_filter)
async def guide_command(_, msg):
    """显示用户指南"""
    await deleteMessage(msg)

    guide_text = f"""
📖 **EmbyBot 用户指南**

╭─────────────────╮
│  🎬 **使用教程**
╰─────────────────╯

**📱 客户端下载：**

**iOS 设备：**
• App Store 搜索 "Emby"
• 或使用 Infuse（推荐）

**Android 设备：**
• Google Play 搜索 "Emby"
• 或各大应用商店

**电脑：**
• 访问 emby.media 下载
• 或使用网页版

**电视/盒子：**
• Android TV 应用商店
• Apple TV App Store

---

**🔧 登录步骤：**

**1. 添加服务器**
   点击"添加服务器"
   输入服务器地址
   （在 🌐 服务器 中查看）

**2. 输入账户**
   用户名：你的 Emby 用户名
   密码：创建账户时的密码

**3. 开始使用**
   浏览媒体库
   播放喜欢的影片

---

**⚙️ 推荐设置：**

**画质设置：**
• WiFi：1080P 或原画
• 移动网络：720P
• 慢速网络：480P

**播放设置：**
• 开启硬件加速
• 启用自动转码
• 调整缓冲时间

**字幕设置：**
• 选择合适的字体大小
• 调整字幕位置
• 设置字幕延迟

---

**💡 使用技巧：**

**1. 收藏功能**
   看到喜欢的就收藏
   方便下次快速找到

**2. 继续观看**
   观看进度自动保存
   跨设备同步

**3. 下载离线**
   WiFi 下下载视频
   随时随地观看

**4. 多设备同步**
   收藏、进度自动同步
   无缝切换设备

---

**🔐 安全提示：**

• 不要分享账户给他人
• 定期更换密码
• 发现异常立即重置
• 定期检查登录设备

---

**📞 需要帮助？**

• 查看 FAQ：`/faq`
• 联系管理员
• 在群组提问
"""

    guide_buttons = ikb([
        [('❓ 常见问题', 'help_faq'),
         ('💬 联系管理', 'help_contact')],
        [('❌ 关闭', 'closeit')]
    ])

    await sendMessage(msg, guide_text, buttons=guide_buttons, timer=180)


# 回调查询处理
@bot.on_callback_query(filters.regex('^help_'))
async def help_callback(_, call):
    """处理帮助相关的回调"""
    action = call.data

    if action == 'help_faq':
        # 显示 FAQ
        await faq_command(_, call.message)

    elif action == 'help_guide':
        # 显示用户指南
        await guide_command(_, call.message)

    elif action == 'help_quickstart':
        # 快速开始
        quickstart_text = f"""
🚀 **快速开始指南**

**步骤 1: 创建账户**
1. 点击 `/start`
2. 点击 🎨 创建账户
3. 输入：`用户名 安全码`
   示例：`张三 1234`
4. 记录密码（仅显示一次）

**步骤 2: 下载客户端**
• iOS: App Store 搜索 "Emby"
• Android: Google Play 搜索 "Emby"
• 电脑: 访问 emby.media

**步骤 3: 登录**
1. 打开 Emby 客户端
2. 添加服务器（在 🌐 服务器 中查看地址）
3. 输入用户名和密码
4. 开始观看

**步骤 4: 续期（重要）**
• 账户有有效期
• 到期前会收到提醒
• 使用续期码：`/exchange 续期码`
• 或在 🏪 兑换商店 兑换

**获取{credits}：**
• 🎯 每日签到
• 👥 邀请好友
• 🎊 参与活动

就这么简单！开始享受吧 🎬
"""
        back_button = ikb([[('↩️ 返回帮助', 'help_back'), ('❌ 关闭', 'closeit')]])
        await call.message.edit(quickstart_text, reply_markup=back_button)

    elif action == 'help_contact':
        # 联系管理
        contact_text = """
💬 **联系支持**

**获取帮助的方式：**

**1. 查看文档**
   • 用户指南：`/guide`
   • 常见问题：`/faq`
   • 帮助命令：`/help`

**2. 群组求助**
   • 在官方群组提问
   • 其他用户会帮助你
   • 管理员会解答

**3. 联系管理员**
   • 在群组 @管理员
   • 或发送私信
   • 说明你的问题

**报告问题时请提供：**
• Telegram ID
• 问题描述
• 截图（如有）
• 复现步骤

**响应时间：**
• 工作时间：1-2 小时
• 非工作时间：可能较慢

我们会尽快帮助你！
"""
        back_button = ikb([[('↩️ 返回帮助', 'help_back'), ('❌ 关闭', 'closeit')]])
        await call.message.edit(contact_text, reply_markup=back_button)

    elif action == 'help_back':
        # 返回主帮助
        await help_command(_, call.message)

    elif action == 'faq_full':
        # 显示完整 FAQ（这里简化处理）
        full_faq_text = """
📚 **完整 FAQ 文档**

完整的常见问题解答文档包含 25+ 个问题的详细解答。

**主要章节：**
• 👤 账户相关（Q1-Q5）
• 📺 使用相关（Q6-Q10）
• 🎟️ 续期相关（Q11-Q15）
• 🔧 技术问题（Q16-Q20）
• ❓ 其他问题（Q21-Q25）

由于篇幅限制，完整文档请：
• 查看群组置顶消息
• 访问文档网站（如有）
• 联系管理员获取

或继续使用 `/faq` 查看常见问题。
"""
        back_button = ikb([[('↩️ 返回 FAQ', 'help_faq'), ('❌ 关闭', 'closeit')]])
        await call.message.edit(full_faq_text, reply_markup=back_button)
